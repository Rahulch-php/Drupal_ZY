...
class GoogleMini {
 ...
 /**
  ...
  public function query($iteratorClass = 'GoogleMiniResultIterator') {
    ...
    $fp = fopen($_SERVER['DOCUMENT_ROOT'].'/sites/default/files/gsa_log.html','w+');
    $string_to_log = "<p>--------------------------------------------------</p><br />";
    $string_to_log.= "<p>Feed Date:- ".gmdate('Y-m-d H:i:s',(time() + 60*60))."</p><br /><p>".$query."</p><br />";
    $string_to_log.= "<p>--------------------------------------------------</p><br />";
    fputs( $fp, $string_to_log, strlen($string_to_log) );
    fclose($fp);
    $ch = curl_init();
    ...
    // get search results in XML using simple XML function
	// $resultXML = simplexml_load_file($query);
	$resultXML = file_get_contents($query);
	//print_r($resultXML);
  ...
  function resultFactory($resultXML,$className = 'GoogleMiniResultIterator') {
    ...
    // $payload = $resultXML;
	//print_r($resultXML); print "</pre>"; 
	$fp = fopen($_SERVER['DOCUMENT_ROOT'].'/sites/default/files/gsa_log.html','a+');
    $string_to_log = "<p>--------------------------------------------------</p><br />";
    $string_to_log.= "<p>Feed Date:- ".gmdate('Y-m-d H:i:s',(time() + 60*60))."</p><br /><p>".$resultXML."</p><br />";
    $string_to_log.= "<p>--------------------------------------------------</p><br />";
    fputs( $fp, $string_to_log, strlen($string_to_log) );
    fclose($fp);
    
    $totalResults = $payload->RES->M;    
    // results count starting and ending limit
    $start_count = $end_count = 0;
	if($totalResults > 0) {
	    foreach($payload->RES->attributes() as $a => $b) {
	      if ($a == "SN") { 
		    $start_count = $b;
		  } 
		  if ($a == "EN") { 
    ...
    $iterator->time = $payload->TM; // execution time
    $iterator->totalResults = $totalResults; // total results
    $iterator->start_count = $start_count; // starting number
    $iterator->end_count = $end_count; // ending number
    $iterator->keyword = $payload->Q; // searched keyword
    return $iterator;
